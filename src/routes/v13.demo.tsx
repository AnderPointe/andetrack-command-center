...
function Page() {
  ...
  const rlsSql = H.useV13RlsSqlSnippets();
  ...
  return (
    <V13Page icon={<ListChecks className="size-6 text-indigo-300" />} title="V13 Demo Flow" blurb="Multi-role capital-readiness walkthrough — CEO, CFO, RevOps, Board admin, MP leader, Partner lead.">
      ...
      <Card className="border-white/10 bg-white/[0.02] p-4">
        ...
        <p className="mt-1 text-xs text-muted-foreground">Illustrative only — see docs/phase39-plan.md and the full V13 RLS sketch list.</p>
        <div className="mt-2 space-y-2">
          {rlsSql.map((r) => (
            <div key={r.table} className="rounded-lg border border-white/10 bg-black/30 p-3 text-sm">
              <div className="font-mono text-xs text-indigo-200">{r.table}</div>
