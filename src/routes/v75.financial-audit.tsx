...
export const Route = createFileRoute("/v75/financial-audit")({
  ...
  component: () => {
    const { audit, trend } = useAdvancedFinancialAuditReadiness();
    return (
      <V75Page icon={<Receipt className="size-6 text-indigo-300" />} title="Advanced Financial Audit Readiness"
        ...
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <ScoreCard label="Placeholder areas"  value={audit.items.filter(i => i.status === "placeholder").length} tone="violet" />
        </div>
