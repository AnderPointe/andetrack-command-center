...
export const Route = createFileRoute("/v75/revenue-recon")({
  ...
  component: () => {
    const { events, summary } = useRevenueReconciliationPlaceholder();
    ...
    return (
      <V75Page icon={<Wallet className="size-6 text-indigo-300" />} title="Revenue Reconciliation Placeholder"
        ...
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <ScoreCard label="Oldest (d)" value={summary.oldest_unmatched_days} tone="violet" />
        </div>
        <KpiGrid cols={3} items={[
          { label: "Unmatched amount", value: `$${summary.unmatched_amount.toFixed(2)}`, sub: "Across 5 events" },
          { label: "Owner",            value: summary.owner },
          { label: "Status",           value: <StatusPill status={summary.status} />, sub: "Placeholder only" },
        ]} />
