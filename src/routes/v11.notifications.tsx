...
import { NOTIF_METRICS, NOTIF_TEMPLATES, NOTIF_FAILURES, NOTIF_TEMPLATE_STATS } from "@/v11/data/mockPhase15";
...
function Page() {
  const statsById = new Map(NOTIF_TEMPLATE_STATS.map((s) => [s.id, s]));
  return (
    <V11Page
    ...
    >
      ...
      <Card className="border-white/10 bg-white/[0.02] p-4">
        <h2 className="text-sm font-semibold">Templates · per-template reliability</h2>
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          {NOTIF_TEMPLATES.map((t) => {
            const s = statsById.get(t.id);
            const delivery = s ? Math.round(s.delivery * 100) : null;
            const open = s ? Math.round(s.open * 100) : null;
            return (
              <div key={t.id} className="flex items-center justify-between rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    ...
                    {delivery !== null && <> · delivery {delivery}% · open {open}%</>}
                  </div>
                </div>
                <div className="flex items-center gap-1.5">
                  ...
                  {delivery !== null && (
                    <Badge variant="outline" className={delivery >= 97 ? "border-emerald-500/30 text-emerald-300" : "border-amber-500/30 text-amber-300"}>
                      {delivery}%
                    </Badge>
                  )}
                </div>
