...
import { CUSTOMER_COMM_DRAFTS, CUSTOMER_COMM_TYPES, COMM_APPROVAL_POLICY } from "@/v25/data/mockPhase18";
...
export const Route = createFileRoute("/v25/communication")({
  ...
  component: () => (
    <V25Page icon={<MessageSquare className="size-6 text-emerald-300" />} title="Customer Communication Automation" blurb="CoPilot drafts customer messages — dispatcher reviews, edits, approves. Per-type approval policy governs auto-draft and channel defaults; nothing sends without explicit approval where required.">
      ...
      <Card className="border-white/10 bg-white/[0.02] p-4">
        ...
        <table className="mt-3 w-full text-sm">
          <thead className="text-left text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="p-2">Type</th><th className="p-2">Auto-draft</th><th className="p-2">Requires approval</th><th className="p-2">Default channel</th></tr></thead>
          <tbody>{COMM_APPROVAL_POLICY.map((p) => (
            <tr key={p.type} className="border-t border-white/10">
              <td className="p-2">{p.type}</td>
              <td className="p-2">{p.autoDraft ? <Badge variant="outline" className="border-emerald-500/30 text-emerald-300">yes</Badge> : <Badge variant="outline" className="border-white/15 text-muted-foreground">no</Badge>}</td>
              <td className="p-2">{p.requiresApproval ? <Badge variant="outline" className="border-amber-500/30 text-amber-300">required</Badge> : <Badge variant="outline" className="border-sky-500/30 text-sky-300">auto-send</Badge>}</td>
              <td className="p-2 text-xs">{p.channelDefault}</td>
            </tr>
          ))}</tbody>
        </table>
      </Card>
