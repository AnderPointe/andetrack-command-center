...
function DemoPage() {
  ...
  return (
    <V85Page icon={<ListChecks className="size-6 text-fuchsia-300" />} title="V8.5 Demo Flow" blurb="17-step executive walkthrough with role guidance and RLS policy references.">
      <ExecBanner h={headline} />
      <Card className="border-white/10 bg-white/[0.02] p-4">
        <h3 className="text-sm font-semibold">Role guidance</h3>
        <div className="mt-2 grid gap-2 md:grid-cols-2 lg:grid-cols-3">
          {guidance.map((g) => (
            <div key={g.role} className={`rounded-lg border ${TONE_MAP[g.tone]} bg-white/[0.02] p-3 text-sm`}>
      ...
      <Card className="border-white/10 bg-white/[0.02] p-4">
        <ol className="space-y-2 text-sm">
          {STEPS.map((s, i) => (
            <li key={i} className="grid grid-cols-[2rem_9rem_1fr_1fr] items-start gap-2 border-b border-white/5 pb-2 last:border-0">
              <span className="text-muted-foreground">{i + 1}.</span>
              <span className="font-medium text-fuchsia-200">{s.who}</span>
              <span>{s.step}</span>
