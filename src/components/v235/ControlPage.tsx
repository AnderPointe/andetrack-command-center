...
import { V235Page } from "./V235Page";
import { ScoreCard, KpiGrid, Section, SimpleTable } from "./ui-bits";

export function ControlPage({ icon, title, blurb, data, scoreLabel = "Maturity score" }: {
  icon: ReactNode; title: string; blurb: string; data: any; scoreLabel?: string;
}) {
  return (
    <V235Page icon={icon} title={title} blurb={blurb}>
      ...
      {data.exceptions?.length ? (
        <Section title="Maturity exceptions">
